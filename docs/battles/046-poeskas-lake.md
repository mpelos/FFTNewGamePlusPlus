...
s4 Revenant (job 114)       L101 — innate monster kit preserved
s5 Revenant (job 114)       L100 — innate monster kit preserved
```

Implemented v3 redesign: preserve the exact vanilla roster, but recognize the
...
```text
Scope: implement battle doc 046 through the embedded ENTD and synchronize the docs.
...
Success bar: patch function, binary diff, validation, deployed build, and direct playtest checklist.
```
...
- Installed JobData confirms jobs 63/70/71 have human equipment categories and innate Float + Undead.
- Explicit ENTD gear, R/S/M, job buckets, JobLevel, levels, and Br/Fa were written to entry 453.
- Job buckets are Mime `19`, Chemist `1`, and Summoner `8`; all four human jobs use JobLevel `8`.
- OverrideEntryData leaves the relevant build fields at `-1`, so the embedded ENTD remains authoritative.
- Main jobs, six-unit count, flags, positions, unit ids, objective, and reward payload were preserved.
...
- Confirm Rubber Suit is active on job 63 in battle; JobData already confirms Clothing legality.
```
...
Keep the exact six-unit roster. Four of six enemies stay at `100`–`101`; only the Mystic and Summoner
anchors reach `102`. Difficulty comes from complete roles plus undead action tax, not a boss band.

| Slot | Role | Main job | Job bucket | Level | Br/Fa | Purpose |
|---|---|---|---|---:|---:|---|
| `s0` | Undead Mystic | Undead Mystic `70` | Mime `L8` | `102` | `72/84` | Durable non-physical pressure; Geomancy + Mana Shield anchor. |
| `s1` | Undead Archer 1 | Undead Archer `63` | Mime `L8` | `101` | `88/70` | Mount-Germinas-style Yoichi crossfire plus ranged Items. |
| `s2` | Undead Archer 2 | Undead Archer `63` | Chemist `L8` | `100` | `88/70` | Durable Yoichi item-support Archer; Rubber Suit + cloak. |
| `s3` | Undead Summoner | Undead Summoner `71` | Summoner `L8` | `102` | `72/84` | Primary AoE clock; Summon + Black Magicks with Swiftspell. |
| `s4` | Revenant apex | Revenant `114` | innate monster kit | `101` | `90/55` | Mobile drain/flank pressure; stronger permakill body. |
| `s5` | Revenant hunter | Revenant `114` | innate monster kit | `100` | `88/55` | Second fast hunter on the opposite approach. |

## Builds

### s0 — Undead Mystic (Lv 102)
...
### s1 — Undead Archer 1 (Lv 101), copied from Mount Germinas
...
### s2 — Undead Archer 2 (Lv 100), Chemist-bucket support Archer
...
deliver Items or secure a new firing lane. It is intentionally the lower-level Archer (`100`).
...
Intuit: fast hunters and permakill pressure. The `101` Revenant pressures the more direct lane; the `100`
...
The implementation subsequently adopted the campaign's revised low-band rule: four units at `100`–`101`
and only the Mystic/Summoner at `102`. This numeric retune was not re-simulated by request and is reserved
for direct in-game validation.
```

## Implementation Checklist

- [x] Confirm entry 453 active slot order: `70,63,63,71,114,114`.
- [x] Preserve Float + Undead on jobs 63/70/71 and Undead on both Revenants.
- [x] Set levels: Mystic `102`, Archer 1 `101`, Archer 2 `100`, Summoner `102`, Revenants `101/100`.
- [x] Set main JobLevel `8` for all four human undead; preserve monster-job handling on Revenants.
- [x] Seed buckets: Mystic→Mime `L8`, Archer 1→Mime `L8`, Archer 2→Chemist `L8`, Summoner→Summoner `L8`.
- [x] Apply the exact secondary/R/S/M and equipment packages documented above.
- [ ] Confirm special jobs 63/70/71 honor explicit ENTD gear and R/S/M fields.
- [x] Do not implement a per-unit learned-skill allowlist; ENTD cannot provide it.
...
- [x] Preserve both Revenant innate kits; do not assign human gear or job buckets to them.
- [x] Preserve guaranteed spoils: Cursed Ring + Phoenix Down + Phoenix Down.
- [x] Preserve buried map treasure and all original terrain/objective behavior.
- [x] Re-dump and diff; all data changes are contained inside entry 453.
- [x] Run the sprite-budget audit; net sprite-sheet change is `+0`.
